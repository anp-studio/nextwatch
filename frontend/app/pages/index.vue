  ...
  <div class="flex flex-col h-screen bg-slate-900 text-white overflow-hidden">
    ...
    <main class="flex-grow relative flex items-center justify-center w-full max-w-md mx-auto p-4">
      ...
      <div v-if="isFinished" class="text-center p-8 animate-fade-in">
        ...
        <button @click="resetDeck" class="bg-white text-slate-900 px-6 py-3 rounded-full font-bold hover:bg-slate-200 transition">
          Refresh Deck
        </button>
      </div>
      ...
      <div v-else class="relative w-full h-[65vh] md:h-[600px]">
        ...
        >
          <img :src="nextMovie.posterUrl" class="w-full h-full object-cover rounded-2xl grayscale" />
        </div>
        ...
        <div 
          ...
          :style="cardStyle"
        >
          <img :src="currentMovie.posterUrl" class="w-full h-full object-cover pointer-events-none" />
          ...
          <div class="absolute bottom-0 w-full p-6">
            ...
            <div class="flex items-center gap-2 mb-3">
              <span class="bg-yellow-400 text-black text-xs font-bold px-1.5 py-0.5 rounded">IMDb {{ currentMovie.imdbRating }}</span>
            ...
            <div class="flex flex-wrap gap-2 mb-4">
              <span v-for="genre in currentMovie.genres" :key="genre" class="text-xs text-white bg-white/20 backdrop-blur-sm px-2 py-1 rounded-md">
                {{ genre }}
              </span>
            </div>
    ...
    <footer v-if="!isFinished" class="p-6 flex justify-center gap-6 z-10 pb-8">
      <button @click="manualSwipe('left')" class="w-14 h-14 bg-slate-800 rounded-full text-red-500 shadow-lg flex items-center justify-center hover:scale-110 transition border border-slate-700">
        <XIcon class="w-8 h-8" />
      </button>

      <button @click="manualSwipe('up')" class="w-10 h-10 mt-2 bg-slate-800 rounded-full text-blue-400 shadow-lg flex items-center justify-center hover:scale-110 transition border border-slate-700">
        <EyeIcon class="w-5 h-5" />
      </button>

      <button @click="manualSwipe('right')" class="w-14 h-14 bg-slate-800 rounded-full text-green-500 shadow-lg flex items-center justify-center hover:scale-110 transition border border-slate-700">
...
import { X as XIcon, Heart as HeartIcon, Eye as EyeIcon } from 'lucide-vue-next'
